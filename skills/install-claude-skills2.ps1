...
<#
.SYNOPSIS
    Install Claude Code Skills from awesome-claude-code and curated repos

.DESCRIPTION
    Clones top skill repos listed in hesreallyhim/awesome-claude-code and
    installs them into ~/.claude/skills/ with namespace prefixes.
    Also installs custom GCP security skills.

    Repos installed:
      acc/    - awesome-claude-code commands (evaluate-repository)
      ...
      ecc/    - Everything Claude Code by affaan-m (comprehensive)
      taches/ - TACHES by glittercowboy (meta-skills, auditor, hooks)
      gcp/    - Custom GCP security (IAM, VPC SC, SCC, Terraform)

.NOTES
    Requires: git in PATH
    Run:      powershell -ExecutionPolicy Bypass -File .\install-claude-skills.ps1
    No admin required.
#>

$SkillsRoot  = Join-Path $env:USERPROFILE ".claude\skills"
$CommandsDir = Join-Path $env:USERPROFILE ".claude\commands"
$TempRoot    = Join-Path $env:TEMP "claude-skills-$([System.IO.Path]::GetRandomFileName())"
$ErrorActionPreference = "Stop"

$Repos = @(
    @("acc",    "https://github.com/hesreallyhim/awesome-claude-code",  "Awesome Claude Code"),
    ...
    @("ecc",    "https://github.com/affaan-m/everything-claude-code",   "Everything Claude Code - Comprehensive"),
...
function Process-Repo {
    ...
    # Pattern 2: .claude/commands/*.md (old slash-command format)
    $cmdDir = Join-Path $CloneDir ".claude\commands"
    if (Test-Path $cmdDir) {
...
# PREFLIGHT
# -----------------------------------------------------------------------
Write-Banner "Claude Code Skills Installer - awesome-claude-code Edition"

Write-Step "Checking prerequisites..."

try {
    $cv = & claude --version 2>&1
    Write-OK "Claude Code: $cv"
} catch {
    Write-Fail "Claude Code not found - install: npm install -g @anthropic-ai/claude-code"
    exit 1
}
...
Write-Host "    acc-    = awesome-claude-code commands" -ForegroundColor White
...
Write-Host "    ecc-    = Everything Claude Code" -ForegroundColor Yellow
...
Write-Host "  Start a new Claude Code session to activate all skills." -ForegroundColor Cyan
Write-Host ""
