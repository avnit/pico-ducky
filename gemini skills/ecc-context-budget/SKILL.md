---
name: context-budget
description: 审核Gemini Code上下文窗口在代理、技能、MCP服务器和规则中的消耗情况。识别膨胀、冗余组件，并提供优先的令牌节省建议。
origin: ECC
---

# 上下文预算

分析 Gemini Code 会话中每个已加载组件的令牌开销，并提供可操作的优化建议以回收上下文空间。
...
| 类别 | 标准 | 操作 |
|--------|----------|--------|
| **始终需要** | 在 GEMINI.md 中被引用，支持活动命令，或匹配当前项目类型 | 保留 |
| **有时需要** | 特定领域（例如语言模式），未在 GEMINI.md 中引用 | 考虑按需激活 |
...
* **冗余组件** — 重复代理逻辑的技能，重复 GEMINI.md 的规则
* **MCP 超额订阅** — >10 个服务器，或包装了可免费使用的 CLI 工具的服务器
* **GEMINI.md 臃肿** — 冗长的解释、过时的部分、本应成为规则的指令
...
```
/context-budget
技能：扫描设置 → 16个代理（12,400个令牌），28个技能（6,200），87个MCP工具（43,500），2个GEMINI.md（1,200）
