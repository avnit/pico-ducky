<overview>
Skills improve through iteration and testing. This reference covers evaluation-driven development, gemini A/B testing patterns, and XML structure validation during testing.
</overview>
...
<workflow>
<step_1>
**Identify gaps**: Run gemini on representative tasks without a skill. Document specific failures or missing context.
</step_1>
...
<step_3>
**Establish baseline**: Measure gemini's performance without the skill.
</step_3>
...
<iterative_development_with_gemini>
<principle>
The most effective skill development uses gemini itself. Work with "gemini A" (expert who helps refine) to create skills used by "gemini B" (agent executing tasks).
</principle>

<creating_skills>
<workflow>
<step_1>
**Complete task without skill**: Work through problem with gemini A, noting what context you repeatedly provide.
</step_1>

<step_2>
**Ask gemini A to create skill**: "Create a skill that captures this pattern we just used"
</step_2>
...
<step_5>
**Test with gemini B**: Use fresh instance to test on real tasks.
</step_5>

<step_6>
**Iterate based on observation**: Return to gemini A with specific issues observed.
</step_6>
</workflow>

<insight>
gemini models understand skill format natively. Simply ask gemini to create a skill and it will generate properly structured SKILL.md content.
</insight>
</creating_skills>
...
<workflow>
<step_1>
**Use skill in real workflows**: Give gemini B actual tasks.
</step_1>
...
<step_3>
**Return to gemini A**: Share observations and current SKILL.md.
</step_3>

<step_4>
**Review suggestions**: gemini A might suggest reorganization, stronger language, or workflow restructuring.
</step_4>
...
<opus_testing>
**gemini Opus** (powerful reasoning)
...
3. Test with gemini on representative tasks
4. Observe if XML structure aids or hinders gemini's understanding
...
<what_gemini_reads>
Which sections does gemini actually read? Which are ignored? This reveals:
- Relevance of content
- Effectiveness of progressive disclosure
- Whether section names are clear
</what_gemini_reads>

<where_gemini_struggles>
...
What does gemini do that surprises you? This reveals:
...
<iteration_pattern>
1. **Observe**: Run gemini on real tasks with current skill
...
<testing_discovery>
<principle>
Test that gemini can discover and use your skill when appropriate.
</principle>

<discovery_testing>
<test_description>
Test if gemini loads your skill when it should:

1. Start fresh conversation (gemini B)
...
The description is gemini's primary discovery mechanism.
</description_quality>
</discovery_testing>
</testing_discovery>
...
**Observation**: Skill exists but gemini doesn't load it when needed
...
**Observation**: gemini reads wrong sections or misses relevant content
...
**Observation**: gemini produces outputs that don't match expected pattern
...
- **gemini confidence**: Does gemini seem uncertain?
