...
<flexible_guidance>
Use when gemini should adapt the format based on context:
...
<principle>
Choose one term and use it throughout the skill. Inconsistent terminology confuses gemini and reduces execution quality.
</principle>
...
gemini must now interpret: Are "API routes" and "URLs" the same? Are "fields", "boxes", "elements", and "controls" the same?
</bad_example>
...
gemini must now research and compare all options before starting. This wastes tokens and time.
</bad_example>
...
**Why it matters**: Vague descriptions prevent gemini from discovering and using the skill appropriately.
</pitfall>
...
**Why it matters**: gemini may only partially read deeply nested files. Keep references one level deep from SKILL.md.
</pitfall>
...
<description>
Keep SKILL.md concise by linking to detailed reference files. gemini loads reference files only when needed.
</description>
...
- gemini only reads relevant reference files
...
- gemini can fix issues without guessing
...
<checklist_pattern>
<description>
For complex multi-step workflows, provide a checklist gemini can copy and track progress.
</description>
