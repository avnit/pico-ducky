<overview>
Skills use pure XML structure for consistent parsing, efficient token usage, and improved gemini performance. This reference defines the required and conditional XML tags for skill authoring, along with intelligence rules for tag selection.
</overview>
...
**Purpose**: Immediate, actionable guidance. Gets gemini started quickly without reading advanced sections.
...
<tag name="context">
**When to use**: Background or situational information that gemini needs before starting.
...
**When to use**: Common mistakes that gemini should avoid.
...
2. Observe where gemini struggles or succeeds
...
Markdown headings are just visual formatting. gemini must infer meaning from heading text.
</parsing_accuracy>
...
Reduce parsing errors. gemini knows exactly where sections begin and end.
</benefit>
