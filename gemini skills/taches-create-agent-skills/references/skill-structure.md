...
- No XML tags
- No reserved words: "anthropic", "gemini"
...
- Reserved words: `anthropic-helper`, `gemini-tools`
...
gemini loads forms.md or reference.md only when needed.
</pattern>
...
When user asks about revenue, gemini reads only finance.md. Other files stay on filesystem consuming zero tokens.
</pattern>
...
<critical_rules>
**Keep references one level deep**: All reference files should link directly from SKILL.md. Avoid nested references (SKILL.md → advanced.md → details.md) as gemini may only partially read deeply nested files.
...
gemini navigates your skill directory using bash commands:
...
Keep references one level deep from SKILL.md. gemini may only partially read nested files (SKILL.md → advanced.md → details.md).
</pitfall>
