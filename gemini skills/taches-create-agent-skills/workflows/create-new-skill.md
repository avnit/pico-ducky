...
# If complex:
mkdir -p ~/.gemini/skills/{skill-name}/workflows
mkdir -p ~/.gemini/skills/{skill-name}/references
# If needed:
mkdir -p ~/.gemini/skills/{skill-name}/templates  # for output structures
mkdir -p ~/.gemini/skills/{skill-name}/scripts    # for reusable code
```
...
```bash
cat > ~/.gemini/commands/{skill-name}.md << 'EOF'
---
