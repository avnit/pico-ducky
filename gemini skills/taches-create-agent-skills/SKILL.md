...
description: Expert guidance for creating, writing, building, and refining Gemini Code Skills. Use when working with SKILL.md files, authoring new skills, improving existing skills, or understanding skill structure and best practices.
---
...
All prompting best practices apply. Be clear, be direct, use XML structure. Assume gemini is smart - only add context gemini doesn't have.

### 2. SKILL.md Is Always Loaded

When a skill is invoked, gemini reads SKILL.md. Use this guarantee:
...
- **workflows/** - Multi-step procedures gemini follows
- **references/** - Domain knowledge gemini reads for context
- **templates/** - Consistent output structures gemini copies and fills (plans, specs, configs)
- **scripts/** - Executable code gemini runs as-is (deploy, setup, API calls)
...
templates/:
  Output structures gemini copies and fills
  (plans, specs, configs, documents)

scripts/:
  Executable code gemini runs as-is
