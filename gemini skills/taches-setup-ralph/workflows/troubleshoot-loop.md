...
gemini --version
```
If not found: `npm install -g @anthropic-ai/gemini-code`

**Check OAuth token (for headless mode):**
```bash
# Verify token exists
cat ~/.gemini-oauth-token

# If missing, run:
gemini setup-token
# Save to ~/.gemini-oauth-token

# Set permissions
chmod 600 ~/.gemini-oauth-token
```
