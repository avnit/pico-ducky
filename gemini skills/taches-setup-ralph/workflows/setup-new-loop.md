...
1. Check `$gemini_CODE_OAUTH_TOKEN` env var
2. Check `~/.gemini-oauth-token` file
3. If neither found, instruct user: "Run `gemini setup-token` and save to ~/.gemini-oauth-token"
...
- Set gemini model (default: `opus` for reasoning, can use `sonnet` for speed)
...
- Then: ./loop-docker.sh
- Requires: ~/.gemini-oauth-token or gemini_CODE_OAUTH_TOKEN env var
- Get token: gemini setup-token
