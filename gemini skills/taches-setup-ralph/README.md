...
A Gemini Code skill that sets up [Ralph Wiggum loops](https://ghuntley.com/ralph/) - Geoffrey Huntley's autonomous AI coding technique.

## What is Ralph?

Ralph is an autonomous coding methodology where gemini runs in a loop:

```bash
while :; do cat PROMPT.md | gemini -p --dangerously-skip-permissions; done
```
...
# Save it
echo "sk-ant-oat01-YOUR-TOKEN" > ~/.gemini-oauth-token
...
Docker mode runs gemini in an isolated container:
- ✅ Can only access the project directory
- ✅ Can't modify system files
- ✅ Uses your gemini Max subscription via OAuth token
