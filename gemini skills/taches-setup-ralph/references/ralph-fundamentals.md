...
The loop continuously feeds a prompt file to Gemini Code CLI. The agent completes one task, updates the implementation plan on disk, commits changes, then exits. The loop restarts immediately with fresh context.
...
The planning prompt instructs gemini to:
...
The building prompt instructs gemini to:
...
The `IMPLEMENTATION_PLAN.md` file is the only state that persists across iterations. This serves as deterministic shared state—no sophisticated orchestration needed. gemini reads it, updates it, commits it.
...
200K advertised tokens ≈ 176K usable tokens. The "smart zone" (where gemini reasons best) is 40-60% of the window.
