#!/bin/bash
# Ralph Docker Loop
# Runs gemini in isolated container, backup/git runs on HOST
...
if [ -z "$gemini_CODE_OAUTH_TOKEN" ]; then
  if [ -f "$TOKEN_FILE" ]; then
    ...
    gemini_CODE_OAUTH_TOKEN=$(cat "$TOKEN_FILE")
  else
...
while true; do
  ...
  if docker run --rm \
    ...
    -w /workspace \
    -e "gemini_CODE_OAUTH_TOKEN=$gemini_CODE_OAUTH_TOKEN" \
    "$IMAGE_NAME" \
    bash -c "cat '$PROMPT_FILE' | gemini --model '$MODEL' -p --dangerously-skip-permissions --output-format text" \
