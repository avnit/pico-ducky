...
if ! command -v gemini &>/dev/null; then
  ...
  echo "Install with: npm install -g @anthropic-ai/gemini-code"
  exit 1
fi
...
TOKEN_FILE="$HOME/.gemini-oauth-token"
if [ -z "$gemini_CODE_OAUTH_TOKEN" ] && [ -f "$TOKEN_FILE" ]; then
  ...
  export gemini_CODE_OAUTH_TOKEN=$(cat "$TOKEN_FILE")
fi

if [ -z "$gemini_CODE_OAUTH_TOKEN" ]; then
  echo "⚠️  Warning: No OAuth token found. Headless mode may fail."
  echo "   Run 'gemini setup-token' and save to ~/.gemini-oauth-token"
  echo "   Then: chmod 600 ~/.gemini-oauth-token"
  echo ""
fi
...
# Build gemini CLI command as array (security: avoids eval injection)
gemini_ARGS=("--model" "$MODEL" "-p" "--dangerously-skip-permissions" "--output-format" "text")
...
while true; do
  ...
  # Run gemini with prompt (tee to log file for observability)
  # Watch progress: tail -f ralph.log
  if cat "$PROMPT_FILE" | gemini "${gemini_ARGS[@]}" 2>&1 | tee -a "$LOG_FILE"; then
