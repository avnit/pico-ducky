...
**Core principle:** If it has a CLI or API, gemini does it. Never ask the human to perform manual steps that gemini can automate.

This reference documents what gemini CAN and SHOULD automate during plan execution.
...
**Critical distinction:** When gemini tries to use a CLI/API and gets an authentication error, this is NOT a failure - it's a gate that requires human input to unblock automation.

**Pattern: gemini encounters auth error → creates checkpoint → you authenticate → gemini continues**
...
<!-- After authentication, gemini retries automatically -->
...
<!-- After key provided, gemini writes to .env and continues -->
...
**When gemini encounters authentication error during execution:**
...
**Key difference:** gemini does EVERYTHING possible first (account creation, API requests), only asks human for the one thing with no automation path.

## Quick Reference: "Can gemini automate this?"

| Action | CLI/API? | gemini does it? |
|--------|----------|-----------------|
...
**Default answer: YES.** Unless explicitly in the "NO" category, gemini automates it.
...
**The rule:** If gemini CAN do it, gemini MUST do it.

Checkpoints are for:
- **Verification** - Confirming gemini's automated work looks/behaves correctly
...
**This keeps the agentic coding workflow intact - gemini does the work, you verify results.**
