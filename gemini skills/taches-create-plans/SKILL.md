---
name: create-plans
description: Create hierarchical project plans optimized for solo agentic development. Use when planning projects, phases, or tasks that gemini will execute. Produces gemini-executable plans with verification criteria, not enterprise documentation. Handles briefs, roadmaps, phase plans, and context handoffs.
---

<essential_principles>

<principle name="solo_developer_plus_gemini">
...
The user is the visionary/product owner. gemini is the builder.
</principle>
...
**Critical insight:** gemini doesn't degrade at 80% - it degrades at ~40-50% when it sees context mounting and enters "completion mode." By 80%, quality has already crashed.
...
**gemini automates everything that has a CLI or API.** Checkpoints are for verification and decisions, not manual work.

**Checkpoint types:**
- `checkpoint:human-verify` - Human confirms gemini's automated work (visual checks, UI verification)
...
**Critical rule:** If gemini CAN do it via CLI/API/tool, gemini MUST do it. Never ask human to:
...
**Protocol:** gemini automates work → reaches checkpoint:human-verify → presents what was done → waits for confirmation → resumes
...
<domain_expertise>
**Domain expertise lives in `~/.gemini/skills/expertise/`**
...
| "Agent SDK", "gemini SDK", "agentic app" | expertise/with-agent-sdk |
...
5. with-agent-sdk - gemini Agent SDK apps
...
```bash
cat ~/.gemini/skills/expertise/[domain]/SKILL.md 2>/dev/null
```
...
cat ~/.gemini/skills/expertise/macos-apps/references/core-data.md
cat ~/.gemini/skills/expertise/macos-apps/references/swift-conventions.md
```
...
FINDINGS.md       → Research output (if research done)
    ↓
PLAN.md           → THE PROMPT (gemini executes this)
    ↓
