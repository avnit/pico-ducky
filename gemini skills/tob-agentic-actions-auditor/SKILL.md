...
description: "Audits GitHub Actions workflows for security vulnerabilities in AI agent integrations including Gemini Code Action, Gemini CLI, OpenAI Codex, and GitHub AI Inference. Detects attack vectors where attacker-controlled input reaches AI agents running in CI/CD pipelines, including env var intermediary patterns, direct expression injection, dangerous sandbox configurations, and wildcard user allowlists. Use when reviewing workflow files that invoke AI coding agents, auditing CI/CD pipeline security for prompt injection risks, or evaluating agentic action configurations."
allowed-tools:
  - Read
  - Grep
  - Glob
...
- Reviewing CI/CD configurations that invoke Gemini Code Action, Gemini CLI, or OpenAI Codex
...
|-----------------|-------------|
| `anthropics/gemini-code-action` | Gemini Code Action |
...
- `gemini_args` -- CLI arguments passed to gemini (may contain `--allowedTools`, `--disallowedTools`)
...
- `settings` -- path to gemini settings file (may configure tool permissions)
...
"Found N AI action instances across M workflow files: X Gemini Code Action, Y Gemini CLI, Z OpenAI Codex, W GitHub AI Inference"
...
4. **Name the specific consequence** in the final step (e.g., "gemini executes with tainted prompt -- attacker achieves arbitrary code execution"), not just the YAML element.
