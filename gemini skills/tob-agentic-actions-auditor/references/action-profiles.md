...
- Bash tool disabled by default; commands must be explicitly allowed via `--allowedTools` in `gemini_args`
...
| Configuration | Risk |
|--------------|------|
| `gemini_args: "--allowedTools Bash(*)"` | Unrestricted shell access; any prompt injection achieves full RCE |
...
```yaml
gemini_args: '--allowedTools "Bash(npm test:*) Bash(git diff:*)"'
```
...
| Remediation Need | Gemini Code Action | OpenAI Codex | Gemini CLI | GitHub AI Inference |
|-----------------|-------------------|--------------|------------|-------------------|
...
| Disable dangerous mode | Remove `Bash(*)` from `gemini_args` | Remove `danger-full-access` from `sandbox` | Remove `--yolo` from CLI args | N/A |
