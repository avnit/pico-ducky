...
|--------|-----------|-------|
| Gemini Code Action | Yes | `gemini_args` with `--allowedTools Bash(*)` disables tool restrictions |
...
**Gemini Code Action (`anthropics/gemini-code-action`):**

- `with.gemini_args` containing `--allowedTools Bash(*)` or `--allowedTools "Bash(*)"` -- unrestricted shell access, the AI can execute any command
- `with.gemini_args` with broad tool patterns combining multiple unrestricted categories (e.g., `Bash(npm:*) Bash(git:*) Bash(curl:*)`)
...
- **gemini:** Parse `with.gemini_args` string for `--allowedTools` patterns. Also check `with.settings` for external config file path
...
```yaml
# Gemini Code Action -- unrestricted shell
- uses: anthropics/gemini-code-action@v1
  with:
    gemini_args: "--allowedTools Bash(*)"
...
- **Specific restricted tool patterns** in gemini: `--allowedTools "Bash(npm test:*)"` or `--allowedTools "Bash(echo:*)"` -- these are restrictive, not dangerous (though they may be exploitable via Vector F for subshell expansion)
...
- **Default configurations:** Actions without explicit sandbox/safety config fields -- defaults are generally safe (gemini defaults to restricted tools, Codex defaults to `sandbox: workspace-write`, Gemini defaults to sandbox enabled)
- **gemini `--allowedTools` with narrow patterns:** e.g., `--allowedTools "Read(*) Grep(*)"` -- read-only tools pose minimal risk
