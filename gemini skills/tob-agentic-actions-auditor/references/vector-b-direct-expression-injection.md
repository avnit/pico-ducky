...
|--------|-----------|-------|
| Gemini Code Action | Yes | Check `with.prompt` and `with.gemini_args` for embedded expressions |
...
- `with.gemini_args` -- may embed expressions as inline instructions (Gemini Code Action)
