...
description: 分析一个陌生的代码库，并生成一个结构化的入门指南，包括架构图、关键入口点、规范和一个起始的GEMINI.md文件。适用于加入新项目或首次在代码仓库中设置Gemini Code时。
origin: ECC
---

# 代码库入门引导

系统性地分析一个不熟悉的代码库，并生成结构化的入门指南。专为加入新项目的开发者或首次在现有仓库中设置 Gemini Code 的用户设计。

## 使用时机

* 首次使用 Gemini Code 打开项目时
* 加入新团队或新仓库时
* 用户询问“帮我理解这个代码库”
* 用户要求为项目生成 GEMINI.md 文件
* 用户说“带我入门”或“带我浏览这个仓库”
...
根据检测到的规范，生成或更新项目特定的 GEMINI.md。如果 `GEMINI.md` 已存在，请先读取它并进行增强——保留现有的项目特定说明，并明确标注新增或更改的内容。
...
**用户**：“带我入门这个代码库”
**操作**：运行完整的 4 阶段工作流 → 生成入门指南 + 初始 GEMINI.md
**输出**：入门指南直接打印到对话中，并在项目根目录写入一个 `GEMINI.md`

### 示例 2：为现有项目生成 GEMINI.md

**用户**：“为这个项目生成一个 GEMINI.md”
**操作**：运行阶段 1-3，跳过入门指南，仅生成 GEMINI.md
**输出**：包含检测到的规范的项目特定 `GEMINI.md`

### 示例 3：增强现有的 GEMINI.md

**用户**：“用当前项目规范更新 GEMINI.md”
**操作**：读取现有 GEMINI.md，运行阶段 1-3，合并新发现
**输出**：更新后的 `GEMINI.md`，并明确标记了新增内容
