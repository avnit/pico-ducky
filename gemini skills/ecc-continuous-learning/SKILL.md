...
description: Gemini Codeセッションから再利用可能なパターンを自動的に抽出し、将来の使用のために学習済みスキルとして保存します。
---

# 継続学習スキル

Gemini Codeセッションを終了時に自動的に評価し、学習済みスキルとして保存できる再利用可能なパターンを抽出します。
...
3. **スキル抽出**: 有用なパターンを`~/.gemini/skills/learned/`に保存
