...
Gemini Codeセッションを信頼度スコアリング付きの小さな学習済み行動である「インスティンクト」を通じて再利用可能な知識に変える高度な学習システム。
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "${gemini_PLUGIN_ROOT}/skills/continuous-learning-v2/hooks/observe.sh pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": "${gemini_PLUGIN_ROOT}/skills/continuous-learning-v2/hooks/observe.sh post"
      }]
    }]
  }
}
...
> 「v1はスキルに依存して観察していました。スキルは確率的で、geminiの判断に基づいて約50-80%の確率で発火します。」
...
v2はv1と完全に互換性があります：
- 既存の`~/.gemini/skills/learned/`スキルは引き続き機能
...
*インスティンクトベースの学習：一度に1つの観察で、geminiにあなたのパターンを教える。*
