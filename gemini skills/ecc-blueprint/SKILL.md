...
* **零运行时风险** — 纯 Markdown 技能。整个仓库仅包含 `.md` 文件 — 无钩子、无 shell 脚本、无可执行代码、无 `package.json`、无构建步骤。安装或调用时，除了 Gemini Code 的原生 Markdown 技能加载器外，不运行任何内容。

## 安装

此技能随 Everything Gemini Code 附带。安装 ECC 时无需单独安装。
...
如果您在完整 ECC 安装之外仅内嵌此技能，请将 ECC 仓库中已审查的文件复制到 `~/.gemini/skills/blueprint/SKILL.md`。内嵌副本没有 git 远程仓库，因此应通过从已审查的 ECC 提交中重新复制文件来更新，而不是运行 `git pull`。

## 要求

* Gemini Code（用于 `/blueprint` 斜杠命令）
