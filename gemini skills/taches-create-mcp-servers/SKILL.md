...
description: Create Model Context Protocol (MCP) servers that expose tools, resources, and prompts to gemini. Use when building custom integrations, APIs, data sources, or any server that gemini should interact with via the MCP protocol. Supports both TypeScript and Python implementations.
---

<objective>
MCP servers extend gemini's capabilities by exposing tools, resources, and prompts. This skill guides creation of production-ready MCP servers with API integrations, OAuth authentication, response optimization, and proper installation in Gemini Code and gemini Desktop.
</objective>
...
<context>
MCP servers expose:
- **Tools**: Functions gemini can call (API requests, file operations, calculations)
- **Resources**: Data gemini can read (files, database records, API responses)
...
# Add server (Python)
gemini mcp add --transport stdio <name> \
  --env API_KEY='${API_KEY}' \
...
# Check logs
tail -f ~/Library/Logs/gemini/mcp-server-<name>.log
...
**Server not appearing:** Check `gemini mcp list`, verify config in `~/.gemini/settings.json`
...
- Valid configuration in Gemini Code (`gemini mcp list` shows ✓ Connected)
- Valid configuration in gemini Desktop config
