...
**End state**: Server running in both Gemini Code and gemini Desktop with all credentials configured.
</overview>
...
- [ ] Step 5: Install in Gemini Code
- [ ] Step 6: Install in gemini Desktop
...
if [ "{language}" = "Python" ]; then
  gemini mcp add --transport stdio {server-name} \
    $ENV_FLAGS \
...
# Verify installation
gemini mcp list | grep {server-name}
```
...
UV_PATH=$(which uv)
NODE_PATH=$(which node)
DESKTOP_CONFIG="$HOME/Library/Application Support/gemini/gemini_desktop_config.json"
...
echo "✓ Installed in gemini Desktop"
echo "⚠️  IMPORTANT: Restart gemini Desktop for changes to take effect"
```
...
**Verify in Gemini Code:**
```bash
# Check server appears
gemini mcp list

# Check logs (if there are issues)
tail -50 ~/Library/Logs/gemini/mcp-server-{server-name}.log
```

**Verify in gemini Desktop:**
1. Restart gemini Desktop
...
**Final checklist:**
```
- [ ] Server appears in `gemini mcp list` with ✓ Connected
- [ ] Environment variables are set in ~/.zshrc
- [ ] Server added to gemini Desktop config
...
gemini mcp list | grep -q "{server-name}" && echo "✓ Installed in Gemini Code" || echo "✗ Not installed"
```

**Step 6 validation:**
```bash
# Check gemini Desktop config
jq '.mcpServers | has("{server-name}")' "$HOME/Library/Application Support/gemini/gemini_desktop_config.json"
# Should output: true
```
...
- gemini Desktop requires absolute paths
...
- gemini Desktop config is backed up automatically
- Can restore with: `cp gemini_desktop_config.json.backup.<timestamp> gemini_desktop_config.json`
