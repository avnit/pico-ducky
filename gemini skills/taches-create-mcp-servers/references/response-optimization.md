...
<critical_pattern>
**Why this matters:** API responses exhaust gemini's context window after just 5-10 operations. Response optimization achieves 85% token reduction and enables 100+ operations per conversation.
...
APIs return verbose responses with nested objects and metadata that gemini doesn't need.
...
```python
# What gemini ACTUALLY needs vs what API returns
FIELD_CONFIGS = {
...
**Key principle:** Include only what gemini needs to:
...
Document available fields in operation schema description or point to API docs. gemini can learn which fields exist through:
...
gemini: [receives 10,000 token response]
User: "Search for Beatles"
gemini: [receives 10,000 tokens]
... after 5-10 searches, context exhausted
gemini: "I've run out of context"
```

**Optimized:**
```
User: "Search for Queen"
gemini: [receives 1,500 token response]
User: "Search for Beatles"
gemini: [receives 1,500 tokens]
User: "List all my playlists"
gemini: [receives first 15k token chunk]
gemini: "Page 1/3 - call continue for more"
User: "continue"
gemini: [receives second chunk from cache]
