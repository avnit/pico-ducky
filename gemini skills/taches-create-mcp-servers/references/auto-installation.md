...
Complete guide for automatically installing MCP servers in both Gemini Code and gemini Desktop with safe credential management.

## Overview

When you build an MCP server, you want it instantly available in both:
- **Gemini Code** - For development and coding workflows
- **gemini Desktop** - For conversational usage
...
1. Adding to Gemini Code via CLI (`gemini mcp add`)
2. Editing gemini Desktop config JSON manually
...
3. Install in Gemini Code (user scope)
4. Manually update gemini Desktop config with variable references
...
### Step 2: Install in Gemini Code

```bash
# Source secrets
source ~/.mcp_secrets

# Install with actual values (Gemini Code stores them securely)
gemini mcp add --transport stdio meta-ads \
  --scope user \
...
**Note:** When using `gemini mcp add`, you pass actual values. Gemini Code stores them securely in `~/.gemini/.gemini.json` and references them correctly.

### Step 3: Configure gemini Desktop

Edit `~/Library/Application Support/gemini/gemini_desktop_config.json`:
...
Restart gemini Desktop and test.
...
**2. Install in Gemini Code:**
```bash
source ~/.mcp_secrets
gemini mcp add --transport stdio stripe \
  --scope user \
  --env STRIPE_API_KEY=${STRIPE_API_KEY} \
  -- uv --directory ~/Developer/mcp/stripe-mcp run python -m src.server
```

**3. Configure gemini Desktop:**
```json
{
  "mcpServers": {
    "stripe": {
...
```bash
source ~/.mcp_secrets
gemini mcp add --transport stdio ghl \
  --scope user \
...
### Check Gemini Code Installation
```bash
# List all installed servers
gemini mcp list

# Get specific server details
gemini mcp get meta-ads

# Remove if needed
gemini mcp remove meta-ads
```

### Check gemini Desktop Configuration

```bash
# View all servers
cat ~/Library/Application\ Support/gemini/gemini_desktop_config.json | jq '.mcpServers'

# Check specific server
cat ~/Library/Application\ Support/gemini/gemini_desktop_config.json | jq '.mcpServers["meta-ads"]'

# Verify cwd property is set
cat ~/Library/Application\ Support/gemini/gemini_desktop_config.json | jq '.mcpServers["meta-ads"].cwd'

# Verify env uses variable expansion
cat ~/Library/Application\ Support/gemini/gemini_desktop_config.json | jq '.mcpServers["meta-ads"].env'
```
...
2. **Install in Gemini Code** using `gemini mcp add` with actual values
3. **Configure gemini Desktop** with variable expansion (`${VAR}`)
...
- Install Gemini Code CLI: Open Gemini Code → run `/install-cli`
...
**"Server not appearing in Gemini Code"**
```bash
# Check installation
gemini mcp list
...
~/.gemini/scripts/install-mcp.sh ...
```

**"Server not appearing in gemini Desktop"**
- Verify JSON syntax: `jq '.' ~/Library/Application\ Support/gemini/gemini_desktop_config.json`
- Check backup file if config is corrupted
- Restart gemini Desktop

**"Environment variable not found"**
- Check `~/.gemini/.env` exists
...
**Gemini Code:**
```bash
gemini mcp add --transport stdio my-ts-server \
  --scope user \
  --env API_KEY=${API_KEY} \
...
```bash
# HTTP server
gemini mcp add --transport http my-server https://api.example.com/mcp

# SSE server with headers
gemini mcp add --transport sse my-server \
...
4. **Variable expansion everywhere** - gemini Desktop configs must use `${VAR}`
