...
Common symptoms:
- Server not appearing in `gemini mcp list`
...
# Check logs
tail -50 ~/Library/Logs/gemini/mcp-server-{name}.log
...
<symptom>Server not in `gemini mcp list`</symptom>
<causes>
- Server never added
- Wrong server name
...
<solution>
Check Gemini Code config:
```bash
cat ~/.gemini/settings.json | jq '.mcpServers'
```
Re-add if missing using `gemini mcp add` command.
</solution>
</issue>
...
gemini mcp add --transport stdio {name} -- /absolute/path/to/uv ...
```
</solution>
</issue>
...
source ~/.zshrc
```
3. Restart Gemini Code to pick up new variables
</solution>
</issue>
...
3. Verify server connects: `gemini mcp list`
4. Check logs are clean: `tail -20 ~/Library/Logs/gemini/mcp-server-{name}.log`
</step>
