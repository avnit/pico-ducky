...
description: "自主gemini代码循环的模式与架构——从简单的顺序管道到基于RFC的多智能体有向无环图系统。"
origin: ECC
---
...
在循环中自主运行 Gemini Code 的模式、架构和参考实现。涵盖从简单的 `gemini -p` 管道到完整的 RFC 驱动的多智能体 DAG 编排的一切。
...
| 模式 | 复杂度 | 最适合 |
|---------|-----------|----------|
| [顺序管道](#1-顺序管道-gemini--p) | 低 | 日常开发步骤，脚本化工作流 |
| [NanoClaw REPL](#2-nanoclaw-repl) | 低 | 交互式持久会话 |
| [无限智能体循环](#3-无限智能体循环) | 中 | 并行内容生成，规范驱动的工作 |
| [持续 gemini PR 循环](#4-持续-gemini-pr-循环) | 中 | 具有 CI 门的跨天迭代项目 |
...
**最简单的循环。** 将日常开发分解为一系列非交互式 `gemini -p` 调用。每次调用都是一个具有清晰提示的专注步骤。

### 核心见解

> 如果你无法想出这样的循环，那意味着你甚至无法在交互模式下驱动 LLM 来修复你的代码。

`gemini -p` 标志以非交互方式运行 Gemini Code 并附带提示，完成后退出。链式调用来构建管道：
...
1. **每个步骤都是隔离的** — 每次 `gemini -p` 调用都是一个新的上下文窗口，意味着步骤之间没有上下文泄露。
...
**ECC 内置的持久循环。** 一个具有会话感知的 REPL，它使用完整的对话历史同步调用 `gemini -p`。
...
1. 从 `~/.gemini/claw/{session}.md` 加载对话历史
2. 每个用户消息都连同完整历史记录作为上下文发送给 `gemini -p`
...
创建 `.gemini/commands/infinite.md`：
...
**一个生产级的 shell 脚本**，在持续循环中运行 Gemini Code，创建 PR，等待 CI，并自动合并。由 AnandChowdhary 创建（致谢：@AnandChowdhary）。
...
│  持续 gemini 迭代                                   │
│                                                     │
│  1. 创建分支 (continuous-gemini/iteration-N)       │
│  2. 使用增强提示运行 gemini -p                      │
│  3. (可选) 审查者通过 — 单独的 gemini -p            │
│  4. 提交更改 (gemini 生成提交信息)                  │
│  5. 推送 + 创建 PR (gh pr create)                   │
│  6. 等待 CI 检查 (轮询 gh pr checks)                │
│  7. CI 失败？ → 自动修复通过 (gemini -p)             │
...
```bash
curl -fsSL https://raw.githubusercontent.com/AnandChowdhary/continuous-gemini/HEAD/install.sh | bash
```
...
gemini 在迭代开始时读取此文件，并在迭代结束时更新它。这弥合了独立 `gemini -p` 调用之间的上下文差距。

### CI 失败恢复

当 PR 检查失败时，持续 gemini 会自动：

1. 通过 `gh run list` 获取失败的运行 ID
2. 生成一个新的带有 CI 修复上下文的 `gemini -p`
3. gemini 通过 `gh run view` 检查日志，修复代码，提交，推送
4. 重新等待检查（最多 `--ci-retry-max` 次尝试）

### 完成信号

gemini 可以通过输出一个魔法短语来发出“我完成了”的信号：
...
2. **连续 gemini + 去草率化** — 为每次迭代添加带有去草率化指令的 `--review-prompt`。
...
2. **迭代之间没有上下文桥接** — 每次 `gemini -p` 调用都从头开始。使用 `SHARED_TASK_NOTES.md` 或文件系统状态来桥接上下文。
