...
# Gemini Code Devcontainer CLI Helper
...
Commands:
    ...
    upgrade             Upgrade Gemini Code to latest version
...
Examples:
    ...
    devc upgrade                # Upgrade Gemini Code to latest
  ...
  custom_mounts=$(jq -c '
    .mounts // [] | map(
      select(
        ...
        (contains("target=/home/vscode/.gemini,") | not) and
...
cmd_upgrade() {
  ...
  check_devcontainer_cli
  log_info "Upgrading Gemini Code..."

  devcontainer exec --workspace-folder "$workspace_folder" gemini update

  log_success "Gemini Code upgraded"
}
...
cmd_update() {
  ...
  if ! git -C "$SCRIPT_DIR" rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    log_info "Re-clone with: rm -rf ~/.gemini-devcontainer && git clone https://github.com/trailofbits/gemini-code-devcontainer ~/.gemini-devcontainer"
    exit 1
  fi
