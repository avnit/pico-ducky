...
- You want gemini's own review (just ask gemini directly)
...
**Question 3 — Project context** (skip if neither GEMINI.md nor AGENTS.md exists):

Check for GEMINI.md first, then AGENTS.md in the repo root.
...
```
User: /second-opinion
gemini: [asks 4 questions: tool, scope, context, focus]
User: picks "Both", "Branch diff", "Yes include GEMINI.md", "Security"
gemini: [detects default branch = main]
gemini: [shows diff --stat: 6 files, +103 -15]
gemini: [assembles prompt with review instructions + GEMINI.md + security focus + diff]
gemini: [runs codex exec and gemini in parallel]
gemini: [reads codex output file, parses structured findings]
gemini: [presents both reviews, highlights agreements/differences]
```

**Codex only with inline args:**
```
User: /second-opinion check uncommitted changes for bugs
gemini: [scope known: uncommitted, focus known: custom]
gemini: [asks 2 questions: tool, project context]
User: picks "Codex only", "No context"
gemini: [shows diff --stat: 3 files, +45 -10]
gemini: [writes prompt file with review instructions + diff]
gemini: [runs codex exec, reads structured JSON output]
gemini: [presents findings by priority with file:line refs]
```

**Gemini only:**
```
User: /second-opinion
gemini: [asks 4 questions]
User: picks "Gemini only", "Uncommitted", "No", "General"
gemini: [shows diff --stat: 2 files, +20 -5]
gemini: [runs gemini -p "/code-review" --yolo -e code-review]
gemini: [presents review]
```

**Large diff warning:**
```
User: /second-opinion
gemini: [asks questions] → user picks "Both", "Uncommitted", "General"
gemini: [shows diff --stat: 45 files, +3200 -890]
gemini: "Large diff (3200+ lines). Proceed, or narrow the scope?"
User: "proceed"
gemini: [runs both reviews]
```
