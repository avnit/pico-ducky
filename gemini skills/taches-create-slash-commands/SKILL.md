...
description: Expert guidance for creating Gemini Code slash commands. Use when working with slash commands, creating custom commands, understanding command structure, or learning YAML configuration.
---

<objective>
Create effective slash commands for Gemini Code that enable users to trigger reusable prompts with `/command-name` syntax. Slash commands expand as prompts in the current conversation, allowing teams to standardize workflows and operations. This skill teaches you to structure commands with XML tags, YAML frontmatter, dynamic context loading, and intelligent argument handling.
</objective>

<quick_start>

<workflow>
1. Create `.gemini/commands/` directory (project) or use `~/.gemini/commands/` (personal)
...
<example>
**File**: `.gemini/commands/optimize.md`
...
gemini receives the expanded prompt and analyzes the code in context.
</example>
</quick_start>
...
**Optional** - Restricts which tools gemini can use
...
**Command file**: `.gemini/commands/fix-issue.md`
```markdown
---
...
**gemini receives**: "Fix issue #123 high-priority following our coding standards"
</all_arguments_string>

<positional_arguments_syntax>

**Command file**: `.gemini/commands/review-pr.md`
```markdown
---
...
**gemini receives**: "Review PR #456 with priority high and assign to alice"
...
gemini can access the referenced file's contents.
</file_references>
...
6. **Save the file**:
   - Project: `.gemini/commands/command-name.md`
   - Personal: `~/.gemini/commands/command-name.md`
</generation_protocol>
