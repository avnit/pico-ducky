---
description: "用于审计gemini技能和命令的质量。支持快速扫描（仅变更技能）和全面盘点模式，采用顺序子代理批量评估。"
origin: ECC
---

# skill-stocktake

斜杠命令 (`/skill-stocktake`)，用于使用质量检查清单 + AI 整体判断来审核所有 gemini 技能和命令。支持两种模式：用于最近更改技能的快速扫描，以及用于完整审查的全面盘点。
...
| 路径 | 描述 |
|------|-------------|
| `~/.gemini/skills/` | 全局技能（所有项目） |
| `{cwd}/.gemini/skills/` | 项目级技能（如果目录存在） |
...
如果项目没有 `.gemini/skills/` 目录，则只评估全局技能和命令。
...
**结果缓存：** `~/.gemini/skills/skill-stocktake/results.json`

## 快速扫描流程

仅重新评估自上次运行以来发生更改的技能（5–10 分钟）。

1. 读取 `~/.gemini/skills/skill-stocktake/results.json`
2. 运行：`bash ~/.gemini/skills/skill-stocktake/scripts/quick-diff.sh \   ~/.gemini/skills/skill-stocktake/results.json`
   （项目目录从 `$PWD/.gemini/skills` 自动检测；仅在需要时显式传递）
3. 如果输出是 `[]`：报告“自上次运行以来无更改。”并停止
4. 使用相同的第 2 阶段标准仅重新评估那些已更改的文件
5. 沿用先前结果中未更改的技能
6. 仅输出差异
7. 运行：`bash ~/.gemini/skills/skill-stocktake/scripts/save-results.sh \   ~/.gemini/skills/skill-stocktake/results.json <<< "$EVAL_RESULTS"`

## 全面盘点流程

### 第 1 阶段 — 清单

运行：`bash ~/.gemini/skills/skill-stocktake/scripts/scan.sh`
...
```
扫描中：
  ✓ ~/.gemini/skills/         (17 个文件)
  ✗ {cwd}/.gemini/skills/    (未找到 — 仅限全局技能)
```
...
```
- [ ] 已检查与其他技能的内容重叠情况
- [ ] 已检查与 MEMORY.md / GEMINI.md 的重叠情况
...
* **独特性**：价值不能被 MEMORY.md / GEMINI.md / 其他技能取代
* **时效性**：技术引用在当前环境中有效
