...
**Missing hook file**: Create `.gemini/hooks.json` or `~/.gemini/hooks.json`

**Invalid JSON**: Use `jq` to validate and format:
```bash
jq . .gemini/hooks.json > temp.json && mv temp.json .gemini/hooks.json
```
...
Submit similar prompt to gemini directly to see response format.
...
### Symptom
Stop hook runs repeatedly, gemini never stops.
...
### Symptom
`$gemini_PROJECT_DIR` or other variables are empty.

### Solutions

**Check variable spelling**:
- `$gemini_PROJECT_DIR` (correct)
- `$gemini_PROJECT_ROOT` (wrong)

**Use double quotes**:
```json
{
  "command": "$gemini_PROJECT_DIR/hooks/script.sh"
}
```
