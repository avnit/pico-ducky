    ...
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"gemini needs your input\" with title \"Gemini Code\" sound name \"Glass\"'"
          }
        ]
      }
    ]
    ...
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "notify-send 'Gemini Code' 'Awaiting your input' --urgency=normal"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '\"[\" + (.timestamp // now | todate) + \"] \" + .tool_input.command + \" - \" + (.tool_input.description // \"No description\")' >> ~/.gemini/bash-log.txt"
          }
        ]
      }
    ]
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '\"[\" + (now | todate) + \"] \" + .tool_name + \": \" + .tool_input.file_path' >> ~/.gemini/file-operations.log"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq '. + {timestamp: now}' >> ~/.gemini/mcp-audit.jsonl"
          }
        ]
      }
    ]
...
sprint_info=$(cat "$gemini_PROJECT_DIR/.sprint-context.txt" 2>/dev/null || echo "No sprint context available")
...
if ! git diff --quiet; then
  git add -A
  git commit -m "chore: auto-commit from gemini session" --no-verify
...
archive_dir="$HOME/.gemini/archives"
mkdir -p "$archive_dir"
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "jq '. + {ended_at: now}' >> ~/.gemini/session-stats.jsonl"
          }
        ]
      }
    ]
...
Use `$gemini_PROJECT_DIR` for project-specific hooks:
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$gemini_PROJECT_DIR/.gemini/hooks/init-session.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$gemini_PROJECT_DIR/.gemini/hooks/validate-changes.sh"
          }
        ]
      }
    ]
