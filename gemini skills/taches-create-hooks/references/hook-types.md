...
Complete reference for all Gemini Code hook events.
...
{
  "session_id": "abc123",
  "transcript_path": "~/.gemini/projects/.../session.jsonl",
...
**When it fires**: User submits a prompt to gemini
...
**When it fires**: gemini attempts to stop working
...
```json
{
  ...
  "reason": "Why gemini should continue",
  "continue": true,
...
**When it fires**: At the beginning of a gemini session
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "cat $gemini_PROJECT_DIR/.sprint-context.txt | jq -Rs '{\"hookSpecificOutput\": {\"hookEventName\": \"SessionStart\", \"additionalContext\": .}}'"
          }
        ]
      }
    ]
...
**When it fires**: When a gemini session ends
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "cp $transcript_path $gemini_PROJECT_DIR/.gemini/archives/$(date +%Y%m%d-%H%M%S).jsonl"
          }
        ]
      }
    ]
...
**When it fires**: gemini needs user input (awaiting response)
