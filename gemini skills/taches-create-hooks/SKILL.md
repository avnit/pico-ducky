---
name: create-hooks
description: Expert guidance for creating, configuring, and using Gemini Code hooks. Use when working with hooks, setting up event listeners, validating commands, automating workflows, adding notifications, or understanding hook types (PreToolUse, PostToolUse, Stop, SessionStart, UserPromptSubmit, etc).
---

<objective>
Hooks are event-driven automation for Gemini Code that execute shell commands or LLM prompts in response to tool usage, session events, and user interactions. This skill teaches you how to create, configure, and debug hooks for validating commands, automating workflows, injecting context, and implementing custom completion criteria.

Hooks provide programmatic control over gemini's behavior without modifying core code, enabling project-specific automation, safety checks, and workflow customization.
</objective>

<context>
Hooks are shell commands or LLM-evaluated prompts that execute in response to Gemini Code events. They operate within an event hierarchy: events (PreToolUse, PostToolUse, Stop, etc.) trigger matchers (tool patterns) which fire hooks (commands or prompts). Hooks can block actions, modify tool inputs, inject context, or simply observe and log gemini's operations.
</context>

<quick_start>
<workflow>
1. Create hooks config file:
   - Project: `.gemini/hooks.json`
   - User: `~/.gemini/hooks.json`
2. Choose hook event (when it fires)
3. Choose hook type (command or prompt)
4. Configure matcher (which tools trigger it)
5. Test with `gemini --debug`
</workflow>
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '\"\\(.tool_input.command) - \\(.tool_input.description // \\\"No description\\\")\"' >> ~/.gemini/bash-log.txt"
          }
        ]
      }
    ]
...
| **Stop** | gemini attempts to stop | Yes |
| **SubagentStop** | Subagent attempts to stop | Yes |
| **SessionStart** | Session begins | No |
| **SessionEnd** | Session ends | No |
| **PreCompact** | Before context compaction | Yes |
| **Notification** | gemini needs input | No |
...
| `${gemini_PLUGIN_ROOT}` | Plugin directory (plugin hooks only) |
    ...
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"gemini needs input\" with title \"Gemini Code\"'"
          }
        ]
      }
    ]
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "prettier --write $gemini_PROJECT_DIR",
            "timeout": 10000
          }
        ]
      }
...
- **Path safety**: Use absolute paths with `$gemini_PROJECT_DIR` to avoid path injection
...
- Valid JSON in `.gemini/hooks.json` (validated with `jq`)
