...
<#
.SYNOPSIS
    Install Gemini Code Skills for Windows 11
    
.DESCRIPTION
    Creates ~/.gemini/skills/ and installs curated SKILL.md packages
...
.NOTES
    ...
    Gemini Code must be installed: npm install -g @anthropic-ai/gemini-code
#>
...
Write-Host "   Gemini Code Skills Installer — GCP Security Edition " -ForegroundColor Magenta
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Magenta

Write-Step "Checking Gemini Code installation..."
try {
    $geminiVersion = & gemini --version 2>&1
    Write-OK "Found Gemini Code: $geminiVersion"
} catch {
    Write-Host "`n  ✗ Gemini Code not found. Install it first:" -ForegroundColor Red
    Write-Host "    npm install -g @anthropic-ai/gemini-code" -ForegroundColor White
    exit 1
}
...
description: >
  ...
  Gemini Code session or another agent. Use when session is getting long, user
...
Write-Host "    Auto-triggered:   just describe what you want, gemini picks the skill" -ForegroundColor Gray
...
Write-Host "    https://github.com/hesreallyhim/awesome-gemini-code" -ForegroundColor Gray
Write-Host ""
Write-Host "  Start a new Gemini Code session to pick up all skills." -ForegroundColor Cyan
Write-Host ""
