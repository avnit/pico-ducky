...
description: 通过Gemini DevFleet协调多智能体编码任务——规划项目、在隔离的工作树中并行调度智能体、监控进度并读取结构化报告。
origin: community
---

# Gemini DevFleet 多智能体编排

## 使用时机

当需要调度多个 Gemini Code 智能体并行处理编码任务时使用此技能。每个智能体在独立的 git worktree 中运行，并配备全套工具。

需要连接一个通过 MCP 运行的 Gemini DevFleet 实例：
